...
class forms{
  ...
  *****************/
  ...
  var $form_open = "<form id=\"<!--ID-->\" name=\"<!--NAME-->\" method=\"POST\" enctype=\"<!--ENCTYPE-->\" onSubmit=\"return validateForm();\" >\n";
  var $form_close = "</form>\n";
  ...
  */
  function buildErrorCSS(){
    $o  = '';
    $o .= "<style type=\"text/css\" scoped=\"scoped\">\n";
    // Field borders
  ...
  */
  ...
  /**
  * NOTE:  The next two functions (showErrorMessages() and showMessages()) have replaced the above two functions for the tuna-neet framework.
  */  
  
  function showErrorMessages(){
    if(count($this->errors) > 0){
      $GLOBALS['error'] = array_merge($GLOBALS['error'],$this->errors);
    }
  }
  
  function showMessages(){
    if(count($this->messages) > 0){
      $GLOBALS['notice'] = array_merge($GLOBALS['notice'],$this->messages);
    }
  }
  ...
  function buildForm($fields=false,$id=false,$name=false,$submit_label='Submit', $validateFormData = true){
    ...
    foreach($fields as $field_id => $field){
      ...
      if($field['type'] != 'html'){
        ...
        $fields[$field_id] = $this->fields[] = $field;
      }
    }
    ...
    foreach($fields as $field_id => $field){
      ...
      // $field['label'] .= ((isset($field['required'])) && ($field['required'] === true)) ? ' [required] ' : ''; // I disabled the [required] part of the label on required fields.
    ...
    // $o .= $this->createFormStateField($name); // This may not be needed at all.
...
//    echo '</pre>';
  ...
  function validateAll(){
    foreach($this->fields as $field_id => $field){
      if(($field['type'] != 'html') && (($field['required'] === true) || (($field['value'] != '') && ($field['value'] != 0)) && (!is_array($field['value'])))){ // Validate if required OR not empty
  ...
  */
  ...
  function SQLEscape($x){
    ...
    return $x; // Escaping should be done at another stage.
  }
